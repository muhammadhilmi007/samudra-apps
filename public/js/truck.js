$(document).ready(function(){
	...
	function index_truck(key, hal, orderby, ascdsc, limit){
		$.ajax({
			url : "/truck/orgdata",
			type : "POST",
			...
			success:function(data){
				...
				if(data.success == 1){
					for(i=0;i<data.data.length;i++){
						var truck = data.data[i];
						$("tbody").append('<tr>'+
								  			'<td>'+truck.id+'</td>'+
								  			'<td>'+truck.no_polisi+'</td>'+
								  			'<td>'+truck.nama_truck+'</td>'+
								  			'<td>'+truck.grup+'</td>'+
								  			'<td>'+
								  				'<a href="/truck/'+truck.id+'/printkbh">'+
								  					'<button type="button" class="btn btn-xs btn-success"><i class="fa fa-print"></i></button>'+
								  				'</a>'+
								  				'<a href="/truck/'+truck.id+'/delete" confirm="Apakah anda yakin akan menghapus truck '+truck.nama_truck+'  ?">'+
								  					'<button type="button" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></button>'+
								  				'</a>'+
								  				'<a href="/truck/'+truck.id+'/edit">'+
	...
	index_truck(key, hal, orderby, ascdsc, limit);
