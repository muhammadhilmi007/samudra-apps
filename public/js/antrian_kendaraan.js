$(document).ready(function(){
	...
	function index_antrian(key, hal, orderby, ascdsc, limit){
		$.ajax({
			url : "/kendaraan/antrian_kendaraan/orgdata",
			type : "POST",
				...
				if(data.success == 1){
					for(i=0;i<data.data.length;i++){
						...
						$("tbody").append('<tr>'+
								  			...
								  			'<td>'+
								  				'<a href="/kendaraan/antrian_kendaraan/'+antrian.id+'/delete" confirm="Apakah anda yakin akan menghapus antrian '+antrian.nama_antrian+'  ?">'+
								  					'<button type="button" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></button>'+
								  				'</a>'+
								  				'<a href="/kendaraan/antrian_kendaraan/'+antrian.id+'/muat" domuat="'+antrian.id+'">'+
						...
						if(antrian.hasMuat == 1){
							...
							$('[domuat='+antrian.id+']').attr("href", "/lansir/"+antrian.s_lansir.id);
						}
					}
	...
	index_antrian(key, hal, orderby, ascdsc, limit);
