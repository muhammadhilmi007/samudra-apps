$(document).ready(function(){
	...
	function index_kendaraan(key, hal, orderby, ascdsc, limit){
		$.ajax({
			url : "/kendaraan/orgdata",
			type : "POST",
			...
			success:function(data){
				...
				if(data.success == 1){
					for(i=0;i<data.data.length;i++){
						var kendaraan = data.data[i];
						$("tbody").append('<tr>'+
								  			'<td>'+kendaraan.id+'</td>'+
								  			'<td>'+kendaraan.no_polisi+'</td>'+
								  			'<td>'+kendaraan.nama_kendaraan+'</td>'+
								  			'<td>'+kendaraan.grup+'</td>'+
								  			'<td>'+kendaraan.s_cabang.nama_cabang+'</td>'+
								  			'<td>'+
								  				'<a href="/kendaraan/'+kendaraan.id+'/delete" confirm="Apakah anda yakin akan menghapus kendaraan '+kendaraan.nama_kendaraan+'  ?">'+
	...
	index_kendaraan(key, hal, orderby, ascdsc, limit);
