$(document).ready(function(){
	...
	function index_muat(key, hal, orderby, ascdsc, limit){
		$.ajax({
			url : "/muat/orgdata",
			type : "POST",
			...
			success:function(data){
				...
				if(data.success == 1){
					for(i=0;i<data.data.length;i++){
						var muat = data.data[i];
						$("tbody").append('<tr>'+
								  			'<td>'+muat.id+'</td>'+
								  			'<td>'+muat.antrian_truck+'</td>'+
								  			'<td>'+muat.waktu_berangkat+'</td>'+
								  			'<td>'+checknull(muat.sampai)+'</td>'+
								  			'<td>'+
								  				'<a href="/muat/'+muat.id+'/delete" confirm="Apakah anda yakin akan menghapus muat '+muat.id+'  ?">'+
	...
	index_muat(key, hal, orderby, ascdsc, limit);
