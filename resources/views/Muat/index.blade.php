...
@section("styles")
	...
	<style>
		@if(!Auth::user()->can("muat:update"))
			.btn_edit{
				display: none;
			}
		@endif

		@if(!Auth::user()->can("muat:delete"))
			.btn-danger{
				display: none;
			}
		@endif
...
@section("content")
	<div class="row">
		@permission("muat:create")
		<div class="col-lg-4">
		  <!-- general form elements -->
		  <div class="box box-primary">
		    <div class="box-header with-border">
		      <h3 class="box-title">Input Data Muat</h3>
		    </div><!-- /.box-header -->
		    <!-- form start -->
		    <form method="POST" action="/muat/add">
		      <div class="box-body">
		      	<div class="form-group">
		          <label>Antrian Truck</label>
		          	<select name="antrian_truck" class="form-control select2">
		          		<option value=""></option>
		          		@foreach($antrian_truck as $at)
		          			<option value="{{$at->id}}">{{$at->id}}|Supir : {{$at->supir}}|Kernet : {{$at->kernet}}</option>
		          		@endforeach
		          	</select>
		      	</div>
		      	<div class="form-group">
		          <label>STT</label>	
		          	<select name="stt[]" class="select2 form-control" multiple="multiple">
		          		...
		          		@foreach($penjualan as $p)
		          			<option value="{{$p->stt}}">id : {{$p->id}} | {{$p->stt}} | K_Asal : {{$p->s_kantor_asal->nama_cabang}} | K_Tujuan : {{$p->s_kantor_tujuan->nama_cabang}}</option>
		          		@endforeach
		          	</select>
		      	</div>
		      	...
		      	<div class="form-group">
		      	  ...
		      	  <select name="cabang_tujuan" class="form-control select2">
		      	  	<option value=""></option>
		      	  	@foreach(\App\Cabang::all() as $c)
		      	  		<option value="{{$c['id']}}">{{$c['nama_cabang']}}</option>
		      	  	@endforeach
		          ...
		          <input type="text" class="form-control datetimepicker" name="waktu_berangkat">
		      	</div>
		        </div>
		...
		@endpermission
		@permission("muat:read")
		<div class="col-lg-8">
			<div class="box box-primary">
			  <div class="box-header with-border">
			    <h3 class="box-title">List Muat</h3>
